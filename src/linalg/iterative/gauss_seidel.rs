...
pub struct GaussSeidelSolver<T, D>
where
...
impl<T, D> GaussSeidelSolver<T, D>
where
    T: IntervalOps + Scalar,
    D: Dim + DimMin<D, Output = D>,
    DefaultAllocator: Allocator<D, D> + Allocator<D>,
{
    /// Default maximum number of Gauss-Seidel iterations.
...
impl<T, D> Solver<T, D> for GaussSeidelSolver<T, D>
where
    T: IntervalOps + Scalar,
    D: Dim + DimMin<D, Output = D>,
    DefaultAllocator: Allocator<D, D> + Allocator<D>,
